...
class Game:
    ...
    def _all_walls(self):
        """Segments de bordure + custom_walls."""
        return self.editor.all_segments() + self.editor.custom_walls

    def _update_enemy_lights(self):
        self.lighting.lights = [l for l in self.lighting.lights if not l.get("_enemy_light")]
    ...
    def _update_fade(self, dt):
        ...
        if self._fade_state=="out":
            ...
            if self._fade_alpha>=255:
                ...
                if self._pending_portal:
                    ...
                    if self.editor.load_map_for_portal(target):
                        ...
                        self._rebuild_grid()
                        if tx>=0 and ty>=0: self.player.rect.x=tx; self.player.rect.y=ty
            ...
            self._fade_alpha-=speed*dt
            if self._fade_alpha<=0: self._fade_alpha=0; self._fade_state="none"
    ...
    def _draw_pause(self):
        w,h=self.screen.get_size()
        overlay=pygame.Surface((w,h),pygame.SRCALPHA); overlay.fill((0,0,0,160)); self.screen.blit(overlay,(0,0))
    ...
    def _handle_pause_key(self, key):
        if key==pygame.K_UP:   self._pause_selection=(self._pause_selection-1)%len(self._pause_options)
        elif key==pygame.K_DOWN: self._pause_selection=(self._pause_selection+1)%len(self._pause_options)
...
>>>>>>> 09ee857c80c2f6be758b182f91793340c5fbe2ee
                ...
                if self.editor.active and self.editor._text_mode is None:
                    if event.type==pygame.MOUSEBUTTONDOWN:
                        if event.button==1:
                            self.editor.handle_click(event.pos); self._rebuild_grid()
                        elif event.button==3:
                            self.editor.handle_right_click(event.pos); self._rebuild_grid()
            ...
            holes     = self.editor.holes       # liste pygame.Rect des trous
            ...
            # Collisions joueur avec tous les segments + custom walls
            ...
            # Trous : on redessine la couleur de fond par-dessus les segments
            # pour rendre le trou visible. Visible uniquement en mode éditeur+hitbox.
            ...
            self.screen.blit(fps_surf,(self.screen.get_width()-fps_surf.get_width()-10,
