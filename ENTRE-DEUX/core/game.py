...
class Game:
    ...
    def _apply_enemy_collisions(self, enemy, all_walls):
        """Applique verifier_collision aux ennemis comme pour le joueur.
        Déclenche saut/demi-tour uniquement sur vrais blocages horizontaux
        (pas sur atterrissages qui bougent légèrement x)."""

        for wall in all_walls:
            if getattr(wall, 'player_only', False): continue
            wr = wall.rect if hasattr(wall, 'rect') else wall
            if not enemy.rect.colliderect(wr): continue

            prev_x      = enemy.rect.x
            prev_bottom = enemy.rect.bottom
            prev_vy     = enemy.vy

            wall.verifier_collision(enemy)

            # Atterrissage vertical
            if enemy.vy == 0 and prev_vy > 0 and enemy.rect.bottom <= prev_bottom:
                enemy.on_ground = True

            # Blocage horizontal : seulement si vy est faible
            # (quand vy est grand = chute/saut → c'est pas un vrai blocage latéral)
            if enemy.rect.x != prev_x and abs(prev_vy) < 80:
                enemy.on_wall_collision_horizontal(wr.height)

        for platform in self.platforms:
            pr = platform.rect
            if not enemy.rect.colliderect(pr): continue

            prev_x      = enemy.rect.x
            prev_bottom = enemy.rect.bottom
            prev_vy     = enemy.vy

            platform.verifier_collision(enemy)

            if enemy.vy == 0 and prev_vy > 0 and enemy.rect.bottom <= prev_bottom:
                enemy.on_ground = True

            if enemy.rect.x != prev_x and abs(prev_vy) < 80:
                enemy.on_wall_collision_horizontal(pr.height)
    ...
    def run(self):
        while self.running:
            ...
            for enemy in self.enemies:
                if enemy.alive:
                    self._apply_enemy_collisions(enemy, all_walls)
